...
function M.view_git_history()
    ...
    vim.cmd("copen")
    vim.api.nvim_buf_set_keymap(0, "n", "<CR>", "<CR><BAR>:lua require('custom.git-diff').diff_current_quickfix_entry()<CR>",
...
function M.diff_current_quickfix_entry()
    ...
    for _, window in ipairs(vim.fn.getwininfo()) do
        if window.winnr ~= vim.fn.winnr() and vim.fn.bufname(window.bufnr) ~= nil and vim.fn.match(vim.fn.bufname(window.bufnr), '^fugitive:') ~= -1 then
    ...
    if qf.idx and type(qf.context) == 'table' and type(qf.context.items) == 'table' then
        ...
        for i = #diff, 1, -1 do
            vim.cmd((i > 1 and 'leftabove' or 'rightbelow') .. ' vert diffsplit ' .. vim.fn.fnameescape(diff[i].filename))
        end
    end
end
